...
class RawCameraRecordService : Service() {
    ...
    private fun startRecording(facing: Int) {
        ...
        val notification = NotificationCompat.Builder(this, "RawCameraChannel")
            .setContentTitle("Raw Camera Recording")
            .setContentText("Recording raw camera at 60fps...")
        ...
        // Determine best resolution for 60fps
        val characteristics = cameraManager.getCameraCharacteristics(cameraId)
        val videoSize = findBest60fpsSize(characteristics)
        Log.i(TAG, "Selected video size: ${videoSize.width}x${videoSize.height}")

        // Setup MediaRecorder
        val timestamp = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(Date())
        currentFile = File(cacheDir, "RawRecord_${videoSize.width}x${videoSize.height}_60fps_$timestamp.mp4")
        ...
        mediaRecorder?.apply {
            ...
            setVideoEncodingBitRate(50_000_000) // 50 Mbps
                ...
                override fun onOpened(camera: CameraDevice) {
                    ...
                    camera.createCaptureSession(surfaces, object : CameraCaptureSession.StateCallback() {
                        override fun onConfigured(session: CameraCaptureSession) {
                            ...
                            mediaRecorder?.start()
                            isRecording = true
                            Log.i(TAG, "Recording started at ${videoSize.width}x${videoSize.height} @ 60fps")
                        }
    ...
    private fun findBest60fpsSize(characteristics: CameraCharacteristics): Size {
        ...
        Log.i(TAG, "Available FPS ranges: ${fpsRanges?.joinToString()}")
        
        val has60fps = fpsRanges?.any { it.upper >= 60 } == true
        
        // Get video sizes from MediaRecorder
        val videoSizes = map.getOutputSizes(MediaRecorder::class.java)
        Log.i(TAG, "Available video sizes: ${videoSizes?.joinToString { "${it.width}x${it.height}" }}")
        
        // Preferred sizes in order: 2K 16:9, 1080p 16:9
        val preferredSizes = listOf(
            Size(2560, 1440),  // 2K
            Size(3840, 2160),  // 4K
            Size(1920, 1080),  // 1080p
            Size(2400, 1080),  // Samsung wide
            Size(1280, 720)    // 720p fallback
        )
        ...
        // Fallback to largest 16:9 size available
        return videoSizes?.filter { 
            val ratio = it.width.toFloat() / it.height.toFloat()
            ratio in 1.7f..1.8f // ~16:9
    ...
    private fun stopRecording() {
        ...
        currentFile?.let { file ->
            if (file.exists() && file.length() > 0) {
                ...
                Log.i(TAG, "Video saved: ${file.name} (${file.length()} bytes)")
            } else {
                Log.w(TAG, "Video file empty or missing")
            }
        }
    }
